...
BUILD_ID = "CHART_INPUTS_ANGLE_PATCH_03"
...
def build_lucy_response(chart: dict) -> dict:
    ...
    sun_mult = angularity_multiplier(sun_deg, angles) if has_angles else 1.0
    moon_mult = angularity_multiplier(moon_deg, angles) if has_angles else 1.0
    mercury_mult = angularity_multiplier(mercury_deg, angles) if has_angles else 1.0
    venus_mult = angularity_multiplier(venus_deg, angles) if has_angles else 1.0
    mars_mult = angularity_multiplier(mars_deg, angles) if has_angles else 1.0
    jupiter_mult = angularity_multiplier(jupiter_deg, angles) if has_angles else 1.0
    saturn_mult = angularity_multiplier(saturn_deg, angles) if has_angles else 1.0
    uranus_mult = angularity_multiplier(uranus_deg, angles) if has_angles else 1.0
    neptune_mult = angularity_multiplier(neptune_deg, angles) if has_angles else 1.0
    pluto_mult = angularity_multiplier(pluto_deg, angles) if has_angles else 1.0

    sun_eff = sun * sun_mult
    moon_eff = moon * moon_mult
    mercury_eff = mercury * mercury_mult
    venus_eff = venus * venus_mult
    mars_eff = mars * mars_mult
    jupiter_eff = jupiter * jupiter_mult
    saturn_eff = saturn * saturn_mult
    uranus_eff = uranus * uranus_mult
    neptune_eff = neptune * neptune_mult
    pluto_eff = pluto * pluto_mult

    capacity = max((sun * 0.80) + (sun_eff * 0.20), 0.01)
    ...
    saturn_constraint = min(
        overload_delta * max(saturn_eff, 0.20) * 0.22,
        amplified_load * 0.50
    )

    net_pressure = amplified_load - regulation
    effective_load = max(amplified_load - (regulation * 0.65) - saturn_constraint, 0.0)
    ...
    timing_pressure = (mars_eff * 0.5) + (jupiter_eff * 0.5)
    timing_mode = (
    ...
    angularity = {
        "sun": sun_mult,
        "moon": moon_mult,
        "mercury": mercury_mult,
        "venus": venus_mult,
        "mars": mars_mult,
        "jupiter": jupiter_mult,
        "saturn": saturn_mult,
        "uranus": uranus_mult,
        "neptune": neptune_mult,
        "pluto": pluto_mult,
    }
